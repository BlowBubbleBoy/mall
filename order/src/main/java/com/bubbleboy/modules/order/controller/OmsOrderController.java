...
import com.bubbleboy.modules.order.excel.OmsOrderExcel;
...
//    @RequiresPermissions("order:omsorder:page")
    public Result<PageData<OmsOrderDTO>> page(@Parameter(hidden = true) @RequestParam Map<String, Object> params){
        PageData<OmsOrderDTO> page = omsOrderService.page(params);

        return new Result<PageData<OmsOrderDTO>>().ok(page);
    }

    @GetMapping("{id}")
    @Operation(summary = "信息")
//    @RequiresPermissions("order:omsorder:info")
    public Result<OmsOrderDTO> get(@PathVariable("id") Long id){
        OmsOrderDTO data = omsOrderService.get(id);

        return new Result<OmsOrderDTO>().ok(data);
    }
...
//    @RequiresPermissions("order:omsorder:save")
    public Result save(@RequestBody OmsOrderDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, AddGroup.class, DefaultGroup.class);

        omsOrderService.save(dto);
...
//    @RequiresPermissions("order:omsorder:update")
    public Result update(@RequestBody OmsOrderDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, UpdateGroup.class, DefaultGroup.class);

        omsOrderService.update(dto);
...
//    @RequiresPermissions("order:omsorder:delete")
    ...
    @LogOperation("导出")
//   @RequiresPermissions("order:omsorder:export")
    public void export(@Parameter(hidden = true) @RequestParam Map<String, Object> params, HttpServletResponse response) throws Exception {
        List<OmsOrderDTO> list = omsOrderService.list(params);

        ExcelUtils.exportExcelToTarget(response, null, "订单", list, OmsOrderExcel.class);
    }
